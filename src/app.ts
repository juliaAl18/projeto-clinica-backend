...
app.post('/api/cadastrar-pagamento', async (req, res) => {
  try {
...
app.post('/api/pacientes', (req, res) => {
  ...
  db.query(
    'INSERT INTO paciente (nome, cpf, dataNascimento, genero, email, telefone, endereco, cidade, estado, cep) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [
      paciente.nome,
      paciente.cpf,
...
app.get('/api/agendamentos', (req, res) => {
  db.query('SELECT * FROM agendamento',
    (error, results) => {
      if (error) throw error;
      res.json(results);
    });
...
app.get('/api/horarios-disponiveis', (req, res) => {
  ...
  db.query(
    'SELECT TIME_FORMAT(hora, "%H:%i") AS hora FROM agendamento WHERE data = ?',
    [data],
    (error, results) => {
      if (error) {
